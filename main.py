...
token = input(f"{colors.red}[{colors.reset}{ok}{colors.red}]{colors.reset} Which Nigga We Fucking Up{colors.red}: {colors.reset}")
...
async def on_connect():
    ctypes.windll.kernel32.SetConsoleTitleW(f'[MULTI TOOL ACCOUNT NUKER] | Connected => {client.user.name}#{client.user.discriminator}')
